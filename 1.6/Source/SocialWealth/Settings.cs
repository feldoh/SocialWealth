...
public class Settings : ModSettings
{
    ...
    public float WealthFactor(Map map)
    {
        return (map ?? Find.AnyPlayerHomeMap).wealthWatcher.WealthTotal / NeutralWealth;
    }
    ...
    public void DoWindowContents(Rect wrect)
    {
        ...
        options.Begin(wrect);
        if (changeThreshold <= 0f)
        {
            changeThreshold = _defaultChangeThreshold;
        }

        if (convertOrDieMinChance < 0f)
        {
            convertOrDieMinChance = _defaultConvertOrDieMin;
        }

        if (convertOrDieMaxChance < 0f)
        {
            convertOrDieMaxChance = _defaultConvertOrDieMax;
        }
        ...
        convertOrDieMinChance = options.SliderLabeled("SocialWealth_Settings_ConvertOrDieMinChance".Translate(convertOrDieMinChance.ToStringPercent()), convertOrDieMinChance, 0f,
            1f,
            tooltip: "SocialWealth_Settings_ConvertOrDieMinChance_Tip".Translate());
        convertOrDieMaxChance = options.SliderLabeled("SocialWealth_Settings_ConvertOrDieMaxChance".Translate(convertOrDieMaxChance.ToStringPercent()), convertOrDieMaxChance, 0f,
            1f,
    ...
    public override void ExposeData()
    {
        ...
        if (neutralWealth <= 0f)
        {
            neutralWealth = _defaultNaturalWealth;
        }
