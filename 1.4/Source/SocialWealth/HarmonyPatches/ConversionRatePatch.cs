        ...
        foreach (CodeInstruction t in codes)
        {
            switch (found)
            {
                ...
                case true when t.opcode == OpCodes.Ret:
                    newCodes.Add(new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ExtraLabelMouseAttachmentTranspiler), nameof(AppendWealthAdjustedCertaintyReduction))));
